#!/usr/bin/env python3
"""
Run factorial experiments for Multi-Objective GFlowNets on DNA Sequences environment.
...
Usage:
    ...
    sudo nice -n -10 python scripts/factorials/sequences/run_factorial_sequences.py \
        --config configs/factorials/sequences_best_config.yaml \
        --output_dir results/factorials/best_configs/sequences_best \
        --resume
...
def run_single_experiment(exp_config: dict,
    ...
    # Create DNASequence environment
    env = DNASequence(
    ...
    # Traditional metrics
    # For sequences, all objectives are normalized to [0, 1], so reference point is just above 1.0
...
def run_factorial_experiment(config_path: Path,
                            ...
                            device: str = 'cpu') -> None:
    """
    Run complete factorial experiment for Sequences environment.
    ...
    print(f"\n{'='*80}")
    print(f"FACTORIAL EXPERIMENT (SEQUENCES): {experiment_name}")
